...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if(document.getElementById("unit-list")){
        ...
        const table = new DataTable('#table-unit', {
          ...
          buttons: [
            ...
            {
              text: '<i class="fa fa-plus"></i> Add Unit',
              className: 'btn btn-primary ms-2 mb-2',
              init: function (dt, node, config) {
                $(node).attr('data-bs-toggle', 'modal');
                $(node).attr('data-bs-target', '#unit-create-modal');
            }
  ...
  $('.unit-edit').on('click',function(){
    ...
    axios.get(`/reg-settings/units/${unitId}/edit`)
    .then((res)=>{
        let {unit} = res.data;
        let form = $('#unit-edit-form');
        form.attr('action', form.attr('action') + '/' + unitId);
        $('#unit_edit').val(unit.unit);
        $('#id').val(unit.id);
        $('#unit-edit-modal').modal('show');
    });
  });
  
    $('.delete-unit').one('click',async function(e){
      const isTrue = await showConfirmation('Do you want to delete this unit?','Delete', 'Cancel');
      if(!isTrue){
        e.preventDefault(); 
        e.stopPropagation();
      }else{
    ...
    $('.default-unit').on('click',function(){
      let unitId = $(this).data('id');
      axios.post(`/reg-settings/units/default`,
      {
        id : unitId, 
