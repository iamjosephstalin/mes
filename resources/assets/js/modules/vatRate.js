...
// List-view screen Js 
  if(document.getElementById("vat-rate-list")){
      const tableTitle = "Vat Rate List"
      const table = new DataTable('#table-vat-rate', {
        responsive: true,
        ...
        buttons: [
          ...
          {
            text: '<i class="fa fa-plus"></i> Add Vat rate',
            className: 'btn btn-primary ms-2 mb-2',
            init: function (dt, node, config) {
              $(node).attr('data-bs-toggle', 'modal');
              $(node).attr('data-bs-target', '#vat-rate-create-modal');
          }
...
$('.vat-rate-edit').on('click',function(){
  let vatRateId = $(this).data('id');
  axios.get(`/reg-settings/vat-rates/${vatRateId}/edit`)
  .then((res)=>{
      let {vatRate} = res.data;
      let form = $('#vat-rate-edit-form');
      form.attr('action', form.attr('action') + '/' + vatRateId);
      $('#vat_rate_edit').val(vatRate.vat_rate);
      $('#id').val(vatRate.id);
      $('#vat-rate-edit-modal').modal('show');
  });
});

  $('.delete-vat-rate').one('click',async function(e){
    const isTrue = await showConfirmation('Do you want to delete this vat rate?','Delete', 'Cancel');
    if(!isTrue){
      e.preventDefault(); 
      e.stopPropagation();
    }else{
  ...
  $('.default-vat-rate').on('click',function(){
    let vatRateId = $(this).data('id');
    axios.post(`/reg-settings/vat-rates/default`,
    {
      id : vatRateId, 
